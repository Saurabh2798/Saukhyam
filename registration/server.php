...
if (isset($_POST['login_user'])) {
  // echo "hi";
  $aadhart = mysqli_real_escape_string($db, $_POST['aadhar']);
  $passwordt = mysqli_real_escape_string($db, $_POST['password']);
   //echo $aadhar;
   //echo $passwordt;
...
echo "hi";
  if (count($errors) == 0) {
    ...
    //echo "after".$password;
    //echo "hi";
    $query = "SELECT * FROM users WHERE aadhar='$aadhar' AND phone='$phonet'";
    // echo $query;
    ...
    //echo $results;
    if (mysqli_num_rows($results) == 1) {
      $_SESSION['aadhar'] = $aadhart;
      $_SESSION['success'] = "You are now logged in";
      header('location: ./Multichain/Transaction.php');
    }else {
      array_push($errors, "Wrong username/password combination");
    }
  }
